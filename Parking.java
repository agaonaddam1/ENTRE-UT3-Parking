...
public class Parking
{
    ...
    public void facturarCliente(char tipoTarifa, int entrada, int salida, int dia) {
        double importe = 0;
        int entradaMinutos = ((entrada/100)*60)+(entrada%100);
        int salidaMinutos = ((salida/100)*60)+(salida%100);
        int onceAMinutos = (11 * 60);
        int tresAMinutos = (3 * 60);
        cliente++;
        String tarifa = "";
        switch(tipoTarifa) {
            case COMERCIAL:
            if ((salidaMinutos - entradaMinutos) < tresAMinutos){
                ...
                importe = (PRECIO_PRIMERAS3_COMERCIAL) +
                ((((salidaMinutos - entradaMinutos)  - tresAMinutos) / 30)* 
            ...
            break;
            case REGULAR:
            if ((entradaMinutos >= HORA_INICIO_ENTRADA_TEMPRANA &&
                entradaMinutos <= HORA_FIN_ENTRADA_TEMPRANA ) && 
            (salidaMinutos >= HORA_INICIO_SALIDA_TEMPRANA &&
                salidaMinutos <= HORA_FIN_SALIDA_TEMPRANA)){
                ...
                if (salida <= 1100){
                    ...
                    (((salidaMinutos - entradaMinutos)/30) * PRECIO_MEDIA_REGULAR_HASTA11); 
                }
                else if (entrada > 1100) {
                    importe = PRECIO_BASE_REGULAR + 
                    (((salidaMinutos - entradaMinutos) / 30) * PRECIO_MEDIA_REGULAR_DESPUES11);
                }
                else if (salida > 1100) {
                    importe = PRECIO_BASE_REGULAR +
                    (((onceAMinutos - entradaMinutos)/30) * PRECIO_MEDIA_REGULAR_HASTA11) +
                    (((salidaMinutos - onceAMinutos)/30) * PRECIO_MEDIA_REGULAR_DESPUES11);
                }
                tarifa = "REGULAR";
            }
            regular++;
    ...
    public void printEstadísticas() {
        ...
        clienteMaximoComercial + " y pagó " + importeMaximoComercial + "€");
        System.out.println("************************************************************");
    }
    ...
    public String diaMayorNumeroClientes() {
        ...
        else if (clientesSabado > clientesLunes && clientesSabado > clientesDomingo){
            dia = "Sabado";
        }
        else if (clientesDomingo > clientesLunes && clientesDomingo > clientesSabado){
            dia = "Domingo";
        }
        else{
            dia = "Ninguno";
